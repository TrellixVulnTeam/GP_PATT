...
namespace tgcalls {
    ...
    namespace {
        ...
        getLines(std::vector<std::string> const &lines, std::string prefix) {
            std::vector<std::string> result;
        ...
        static std::string
        createSdp(uint32_t sessionId, GroupJoinResponsePayload const &payload, bool isAnswer,
        ...
        static std::string
        parseJoinResponseIntoSdp(uint32_t sessionId, uint32_t mainStreamAudioSsrc,
                                 GroupJoinResponsePayload const &payload, bool isAnswer,
                                 std::vector<uint32_t> const &allOtherSsrcs,
        ...
        public:
            ...
            virtual webrtc::FrameDecryptorInterface::Result Decrypt(cricket::MediaType media_type,
                ...
                return webrtc::FrameDecryptorInterface::Result(
                        webrtc::FrameDecryptorInterface::Status::kOk, encrypted_frame.size());
            }
        ...
        public:
            PeerConnectionObserverImpl(
                    ...
                    std::function<void(
                            rtc::scoped_refptr<webrtc::RtpTransceiverInterface>)> onTrackAdded,
                    std::function<void(
                            rtc::scoped_refptr<webrtc::RtpReceiverInterface>)> onTrackRemoved,
            ...
            virtual void
            OnRemoveStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream) override {
            }
            ...
            virtual void
            OnIceCandidateError(const std::string &host_candidate, const std::string &url,
            ...
            virtual void
            OnIceCandidatesRemoved(const std::vector<cricket::Candidate> &candidates) override {
            }
            ...
            virtual void
            OnRemoveTrack(rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver) override {
        ...
        public:
            RTCStatsCollectorCallbackImpl(std::function<void(
                    const rtc::scoped_refptr<const webrtc::RTCStatsReport> &)> completion) :
        ...
        private:
            std::function<void(
                    const rtc::scoped_refptr<const webrtc::RTCStatsReport> &)> _completion;
        };
        ...
        public:
            ...
            bool update(webrtc::AudioBuffer *buffer) {
                webrtc::AudioFrameView<float> frameView(buffer->channels(), buffer->num_channels(),
                                                        buffer->num_frames());
        ...
        public:
            ...
            virtual void OnData(const void *audio_data, int bits_per_sample, int sample_rate,
        ...
        public:
            SetSessionDescriptionObserverImpl(std::function<void()> completion,
                                              std::function<void(webrtc::RTCError)> error) :
        ...
        public:
            ...
            virtual int32_t
            PlayoutDeviceName(uint16_t index, char name[webrtc::kAdmMaxDeviceNameSize],
                              char guid[webrtc::kAdmMaxGuidSize]) override {
                return _impl->PlayoutDeviceName(index, name, guid);
            }

            virtual int32_t
            RecordingDeviceName(uint16_t index, char name[webrtc::kAdmMaxDeviceNameSize],
                                char guid[webrtc::kAdmMaxGuidSize]) override {
    ...
    public:
        ...
        void generateAndInsertFakeIncomingSsrc() {
            ...
            while (true) {
                ...
                if (_fakeIncomingSsrc != 0
                    ...
                    && std::find(_allOtherSsrcs.begin(), _allOtherSsrcs.end(), _fakeIncomingSsrc) ==
                       _allOtherSsrcs.end()) {
                    break;
                }
            }
        ...
        bool createAudioDeviceModule(
            ...
            _adm_thread->Invoke<void>(RTC_FROM_HERE, [&] {
                ...
                if (auto result = check(webrtc::AudioDeviceModule::kPlatformDefaultAudio)) {
                    _adm_use_withAudioDeviceModule = new rtc::RefCountedObject<WrappedAudioDeviceModule>(
                            result);
#ifdef WEBRTC_LINUX
        ...
        void start() {
            ...
            auto analyzer = new AudioCaptureAnalyzer(
                    [&, weak, myVad](const webrtc::AudioBuffer *buffer) {
                        if (!buffer) {
                            return;
                        }
                        ...
                        getMediaThread()->PostTask(RTC_FROM_HERE,
                                                   [weak, peak, peakCount, vadStatus]() {
                                                       ...
                                                       if (strong->_myAudioLevelPeakCount >= 1200) {
                                                           float level = strong->_myAudioLevelPeak /
                                                                         4000.0f;
                                                           if (strong->_isMuted) {
                                                               level = 0.0f;
                                                           }
...
//        config.
            ...
            _peerConnection = _nativeFactory->CreatePeerConnection(config, nullptr, nullptr,
                                                                   _observer.get());
...
//            beginLevelsTimer(50);
        ...
        void emitJoinPayload(std::function<void(GroupJoinPayload)> completion) {
            ...
            rtc::scoped_refptr<CreateSessionDescriptionObserverImpl> observer(
                    new rtc::RefCountedObject<CreateSessionDescriptionObserverImpl>(
                            [weak, completion](std::string sdp, std::string type) {
                                getMediaThread()->PostTask(RTC_FROM_HERE,
                                                           [weak, sdp, type, completion]() {
                                                               auto strong = weak.lock();
                                                               if (!strong) {
                                                                   return;
                                                               }
                                                               ...
                                                               if (sessionDescription != nullptr) {
                                                                   rtc::scoped_refptr<SetSessionDescriptionObserverImpl> observer(
                                                                           new rtc::RefCountedObject<SetSessionDescriptionObserverImpl>(
                                                                                   [weak, adjustedSdp, completion]() {
                                                                                       auto strong = weak.lock();
                                                                                       if (!strong) {
                                                                                           return;
                                                                                       }
                                                                                       auto payload = parseSdpIntoJoinPayload(
                                                                                               adjustedSdp);
                                                                                       if (payload) {
                                                                   ...
                                                                   strong->_peerConnection->SetLocalDescription(
                                                                           observer,
                                                                           sessionDescription);
                                                               } else {
                                                                   return;
        ...
        void removeSsrcs(std::vector<uint32_t> ssrcs) {
            ...
            bool updated = false;
            for (auto ssrc : ssrcs) {
                if (std::find(_allOtherSsrcs.begin(), _allOtherSsrcs.end(), ssrc) !=
                    _allOtherSsrcs.end() &&
                    std::find(_activeOtherSsrcs.begin(), _activeOtherSsrcs.end(), ssrc) !=
                    _activeOtherSsrcs.end()) {
            ...
            if (updated) {
                auto sdp = parseJoinResponseIntoSdp(_sessionId, _mainStreamAudioSsrc,
                                                    _joinPayload.value(), false, _allOtherSsrcs,
                                                    _activeOtherSsrcs);
                setOfferSdp(sdp, false, false, false);
            }
        }
        ...
        void applyLocalSdp() {
            ...
            rtc::scoped_refptr<CreateSessionDescriptionObserverImpl> observer(
                    new rtc::RefCountedObject<CreateSessionDescriptionObserverImpl>(
                            [weak](std::string sdp, std::string type) {
                                getMediaThread()->PostTask(RTC_FROM_HERE, [weak, sdp, type]() {
                                    ...
                                    for (auto &line : lines) {
                                        ...
                                        if (adjustedLine.find("a=ssrc:") == 0) {
                                            ...
                                            if (i >= startIndex) {
                                                adjustedLine.replace(startIndex, i - startIndex,
                                                                     generatedSsrcString);
                                            }
                                        }
                                    ...
                                    webrtc::SdpParseError error;
                                    webrtc::SessionDescriptionInterface *sessionDescription = webrtc::CreateSessionDescription(
                                            type, adjustLocalDescription(adjustedSdp), &error);
                                    if (sessionDescription != nullptr) {
                                        rtc::scoped_refptr<SetSessionDescriptionObserverImpl> observer(
                                                new rtc::RefCountedObject<SetSessionDescriptionObserverImpl>(
                                                        [weak, adjustedSdp]() {
                                                            ...
                                                            auto sdp = parseJoinResponseIntoSdp(
                                                                    strong->_sessionId,
                                                                    strong->_mainStreamAudioSsrc,
                                                                    strong->_joinPayload.value(),
                                                                    true, strong->_allOtherSsrcs,
                                                                    strong->_activeOtherSsrcs);
                                                            strong->setOfferSdp(sdp, false, true,
                                                                                false);
                                                        }, [](webrtc::RTCError error) {
                                                        }));
                                        strong->_peerConnection->SetLocalDescription(observer,
                                                                                     sessionDescription);
                                    } else {
                                        return;
                                    }
        ...
        void setOfferSdp(std::string const &offerSdp, bool isInitialJoinAnswer, bool isAnswer,
                         bool completeMissingSsrcSetup) {
            ...
            webrtc::SdpParseError error;
            webrtc::SessionDescriptionInterface *sessionDescription = webrtc::CreateSessionDescription(
                    isAnswer ? "answer" : "offer", adjustLocalDescription(offerSdp), &error);
            ...
            rtc::scoped_refptr<SetSessionDescriptionObserverImpl> observer(
                    new rtc::RefCountedObject<SetSessionDescriptionObserverImpl>(
                            [weak, isInitialJoinAnswer, isAnswer, completeMissingSsrcSetup]() {
                                getMediaThread()->PostTask(RTC_FROM_HERE,
                                                           [weak, isInitialJoinAnswer, isAnswer, completeMissingSsrcSetup]() {
                                                               auto strong = weak.lock();
                                                               if (!strong) {
        ...
        void collectStats() {
            ...
            rtc::scoped_refptr<RTCStatsCollectorCallbackImpl> observer(
                    new rtc::RefCountedObject<RTCStatsCollectorCallbackImpl>(
                            [weak](const rtc::scoped_refptr<const webrtc::RTCStatsReport> &stats) {
...
//                    streamId.replace(0, 5, "");
                    ...
                    if (_audioTrackSinks.find(ssrc) == _audioTrackSinks.end()) {
                        const auto weak = std::weak_ptr<GroupInstanceManager>(shared_from_this());
                        std::shared_ptr<AudioTrackSinkInterfaceImpl> sink(
                                new AudioTrackSinkInterfaceImpl(
                                        [weak, ssrc](float level, bool hasSpeech) {
                                            getMediaThread()->PostTask(RTC_FROM_HERE,
                                                                       [weak, ssrc, level, hasSpeech]() {
                                                                           auto strong = weak.lock();
                                                                           if (!strong) {
                                                                               return;
                                                                           }
                                                                           auto current = strong->_audioLevels.find(
                                                                                   ssrc);
                                                                           if (current !=
                                                                               strong->_audioLevels.end()) {
        ...
        void emitAnswer(bool completeMissingSsrcSetup) {
            ...
            rtc::scoped_refptr<CreateSessionDescriptionObserverImpl> observer(
                    new rtc::RefCountedObject<CreateSessionDescriptionObserverImpl>(
                            [weak, completeMissingSsrcSetup](std::string sdp, std::string type) {
                                getMediaThread()->PostTask(RTC_FROM_HERE,
                                                           [weak, sdp, type, completeMissingSsrcSetup]() {
                                                               auto strong = weak.lock();
                                                               if (!strong) {
                                                                   return;
                                                               }
                                                               ...
                                                               webrtc::SessionDescriptionInterface *sessionDescription = webrtc::CreateSessionDescription(
                                                                       type,
                                                                       adjustLocalDescription(sdp),
                                                                       &error);
                                                               if (sessionDescription != nullptr) {
                                                                   rtc::scoped_refptr<SetSessionDescriptionObserverImpl> observer(
                                                                           new rtc::RefCountedObject<SetSessionDescriptionObserverImpl>(
                                                                                   [weak, sdp, completeMissingSsrcSetup]() {
                                                                                       auto strong = weak.lock();
                                                                                       if (!strong) {
                                                                                           return;
                                                                                       }
    ...
    GroupInstanceImpl::GroupInstanceImpl(GroupInstanceDescriptor &&descriptor)
        ...
        _manager.reset(new ThreadLocalObject<GroupInstanceManager>(getMediaThread(),
                                                                   [descriptor = std::move(
                                                                           descriptor)]() mutable {
                                                                       return new GroupInstanceManager(
                                                                               std::move(
                                                                                       descriptor));
                                                                   }));
